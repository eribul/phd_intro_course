...
\item[Hip osteoarthritis (80 \%): ]{THA is an elective treatment for tese
...
individuals (such as proffesional athletes).}
...
current practice of handling missing comorbidity data for prediction of
mortality? 
4. Can we also put all this into practice by contributing with
software tools to facilitate patient classification of comorbidity and other
relevant data? 
...
Welfare (Socialstyrelsen) who supplied us with comorbidity data from the
...
residence, sociodemographics, dates of surgery, possible comorbidities and date
of death} 
...
fracture. This affects mostly old and fragile patients for whom comorbidity
...
(as identified by similar methodology as comorbidities, but after, and not
...
draft of the research plan. I therefore had the chance to include a section on
theory of science (along with ethics) already from the start.
...
logic. By so, it shares common ground with theoretical philosophy by the
internal use of deduction and not induction.

The statistical methods here work as a link between the deductive mathematics
and the inductive medical science. What I here refer to as ''induction'' is
therefore identical to ''statistical inference''. (The term does in fact have
...
generalize some of the results also to other countries with similar health care
system as ours.

A long term goal is also to facilitate predictions useful to future patients.
Extrapolation as such might not be completely justified by any scientific
framework, but it does nevertheless make us dependent on inductive reasoning.
...
falsified, not necessarly that it should be formally tested within each study.
...
estimation of a so called ''concordance index'' as a meassure of predictive power,
...
comorbidity data is however on a scale close to ''big data''. Available computer
resources is therefore a limiting factor making it difficult to achieve such
estimates in practive. We therefore plan to include a more practical software
...
goal is to provide both the patient, but also the treating physician, with a
shared decision making tool. It is our goal to use readily available patient
data for the basis of a recommendation whether to operate or not. We hope that
this will reduce the number of THA's causing more harm than good, and likewise
...
comorbidities (from the Swedish patient register), medical prescriptions (from
...
register as an administrative source for payments, sometimes causing skewed
...
comorbidities, just to study their future mortality.}
...
a very limited options of animal models. Second, the required scale of such an
...
information not only regarding fixed comorbidity classes but also to get a
deeper understanding of perceived health status and severity of each coexisting
comorbidity. It would also have the potential to give a deeper understanding of
feelings and the reasoning behind a patients decision to accept or decline the
offer of THA. Similar questions would be relevant also for treating physicians regarding their strategies of patient communication and reasoning when offering such procedure. Although relevant, this is however outside the scope of the current project.}
...
I did explore some ethical issues already in the research plan above. I was
there focused on the research process itself and not on its potential
outcome.
...
A possible misuse of a shared decision making tool would be if policy makers
...
To avoid such misuse can of course never be guaranteed but we do 
...
for the research process itself. We are allowed to use the data described, to build statistical models. 
Current legislation does however not permit that future patients' register data is used 
directly for a clinical decision through a decision tool 
(since the quality register data is not authorised  
the same way as a medical journal). It is therefore necessary for the treating physician 
to double-enter data for the shared decision making tool (even though the
same data might be already available from the register). This might be
cumbersome to the doctor but it might also make it a little more transparent to
...
arthroplasty'' or ''hip replacements'') are not fully standardized. The idea of
manually indexed articles feels appealing. I must admit I first hesitated to use
the database, mainly due to its old-style graphical design.} 

\item[Scopus: ]{my
field is not entirely covered by the field of medicine. This is not obvious from
the search queries above (focused on medical applications). Scopus does however
cover journals of statistics (mathematical science) which PubMed does not.} 

\item[Google Scholar: ]{has previously been my go-to search engine and is also
used extensively by my super-visor. I have so far not been so concerned by the
sources of indexed journals et cetera since I have hoped to be able to deduct
...
after THA). I therefore started to exclude ''hospital mortality, which I found as
a hopefully relevant MeSH term (with `NOT`, `AND NOT` and `-` in the different
data bases)''. I did find this too narrow and therefore dropped it. 
* I also
...
PubMed for this purpose and it might be good to rely on the curated list of
journals.} 
...
but might still be found by Google. I will probably still use Google Scholar
...
The articles found above very actually not that many. To get immediate alerts
for newly indexed items therefore seems relevant. To create such alert for all
sources would probably be redundant. I would therefore choose to create alerts
